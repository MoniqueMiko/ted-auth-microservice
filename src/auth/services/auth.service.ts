...
export class AuthService {
    ...
    async store(data) {
        ...
        if (validate?.status === 400) return await this._httpException.responseHelper(400, await validate.message);

        const hashed = await bcrypt.hash(data.password, 10);

        const userExist = await this._user.findOne({ where: { email: data.email }, });
        if (userExist) return await this._httpException.responseHelper(409, 'Email already exists');
    ...
    async login(data) {
        ...
        if (validate?.status === 400) return await this._httpException.responseHelper(400, validate.message,);

        const user = await this._user.findOne({ where: { email: data.email } });
        if (!user || user === null || user === undefined) return await this._httpException.responseHelper(401, 'Email not found');

        const valid = await bcrypt.compare(data.password, user.password);
        if (!valid) return await this._httpException.responseHelper(401, 'Invalid password',);

        const jwt = await this.generateJwt(user.id, user.email)

        return await this._httpException.responseHelper(200, { jwt: jwt.access_token, email: user.email, name: user.fullName });
    }
